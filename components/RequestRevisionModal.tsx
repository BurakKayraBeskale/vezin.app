...
export default function RequestRevisionModal({ taskId, onClose, onSubmitted }: Props) {
  ...
  async function handleSubmit() {
    if (!note.trim()) {
      setError("Revizyon notu zorunludur");
      return;
    }
    setLoading(true);
    setError("");
    try {
      const payload: Record<string, unknown> = { action: "request_revision", reviewNote: note.trim() };
      ...
      if (!res.ok) {
        setError(data.error || "Revizyon isteği gönderilemedi");
        return;
      }
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[95vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-800">Revizyon İste</h2>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Revizyon Notu *</label>
            <textarea
              rows={4}
              value={note}
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {loading ? "Gönderiliyor..." : "Revizyon İste"}
            </button>
          </div>
        </div>
      </div>
